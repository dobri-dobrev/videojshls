...
videojs.HlsHandler.prototype.src = function(src) {
  ...
  this.playlists.on('loadedmetadata', function() {
    ...
    if (oldMediaPlaylist.endList &&
      ...
      // console.log("THIS IS NOT A LIVE VIDEO");
  ...
  this.playlists.on('error', function() {
    console.log("playlist threw an error");
...
videojs.HlsHandler.prototype.fillBuffer = function(mediaIndex) {
  ...
  //VERY HACKY, probably bad
  if(CURRENT_TIME_HOYDAA_ === 0){
    CURRENT_TIME_HOYDAA_ = this.tech_.currentTime();
  }
  // if preload is set to "none", do not download segments until playback is requested
  if (this.loadingState_ !== 'segments') {
    console.log("preload is set to none");
    return;
  }
  ...
  if (this.pendingSegment_) {
    console.log("buffer is being updated");
    return;
  }
  ...
  if (this.playlists.state === 'SWITCHING_MEDIA') {
    console.log("Playlist switch in progress");
    return;
  }
  ...
  if (mediaIndex === undefined) {
    if (currentBuffered && currentBuffered.length) {
      ...
      mediaIndex = this.playlists.getMediaIndexForTime_(CURRENT_TIME_HOYDAA_);
    }
  }
  ...
  if (this.lastSegmentLoaded_ &&
    ...
    console.log("fetching the same segment");
    return;
  }
...
videojs.HlsHandler.prototype.loadSegment = function(segmentInfo) {
  ...
  console.log("segment duration is " + segment.duration);
  console.log(segmentInfo.uri);
  this.segmentXhr_ = videojs.Hls.xhr({
    ...
    console.log("adding " + segment.duration + " to current time");
    CURRENT_TIME_HOYDAA_ += segment.duration;
    console.log(CURRENT_TIME_HOYDAA_);
    self.drainBuffer();
